...
def main_wrapper_function(database_name, reference_seqs, reference_tax):
    ...
    pipelines = {'q2-NB': build_pipeline(MultinomialNB, hash_params, classify_params)}

    sweep = gen_param_sweep(data_dir, results_dir, reference_dbs, dataset_reference_combinations, method_parameters_combinations)
    sweep = list(sweep)
    
    train_and_run_classifier(method_parameters_combinations, reference_dbs, pipelines, sweep, verbose=True, n_jobs=1)

    taxonomy_glob = join(results_dir, 'mock-*', database_name, 'q2-NB', '*', 'taxonomy.tsv')
